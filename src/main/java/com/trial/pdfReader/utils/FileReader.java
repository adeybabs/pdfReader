package com.trial.pdfReader.utils;
...
/**
 * I kind of prefer for the functionality that reads the file to be in the utility package. You can do otherwise. <br>
 * I will try to give a breakdown on how this works, and yes this can be done in a better fashion, but I will leave it up to you.
 *
 * While I believe this will work for you, it can be done even better and organized better, it's all up to you. Cheers
 *
 * @author chriseteka
 */
@Service
public class FileReader {
    ...
    public SampleModel read(final String pathToFile, final FileConfig fileConfig) {
        ...
        try {
            ...
            //2. Load the document, given the file path to the document of interest
            final PDDocument document = PDDocument.load(new File(pathToFile));

            //3. Instantiate the PDF Stripper which we shall use to read the document
            final PDFTextStripper pdfStripper = new PDFTextStripper();

            //4. Retrieve text from the loaded PDF document, at this point, the whole document is read and arranged like a string in multi lines
            ...
            //5. We first turn the multi-lines text into an array, the element of this array is therefore each of the single line
            ...
            //6. Prepare variables like the array size, a string builder, so we can gather all the unwanted text,
            // and a container list which will hold all the individual line of transaction found in the PDF file (this is important)
            final int textSplitLength = textSplit.length;
            final StringBuilder metaBuilder = new StringBuilder();
            final List<Transaction> transactions = new ArrayList<>();

            //7. Now we iterate through the array holding the individual lines extracted from the PDF, for each line we pick,
            ...
            //Lastly, we add the list of transactions in the JSON model we are building, and also add the other junk information into it
            ...
            //This may not be ideal, but I did it anyway, it's on you to do what you want
            e.printStackTrace();
     ...
     * which is a data structure design to hold monetary value. Read and understand how this is achieved
     * @param data This is the String we want to return it's actual monetary value
     * @return the value passed as BigDecimal, this could be zero in a case where a useless text is passed
     */
